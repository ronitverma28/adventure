...
public class TrekServiceImpl implements TrekService {
    ...
    public List<TrekAvailabilityResponse> getAvailability(Long id) {
        ...
        List<TrekAvailabilityResponse> availability = new ArrayList<>(
                batchRepository.findByTrekIdAndStartDateGreaterThanEqualAndStatusOrderByStartDateAsc(
                        trek.getId(),
                        LocalDate.now(),
...
//            .priceChild(trek.getPriceChild())
                ...
                .metaDescription(trek.getMetaDescription()).highlights(trek.getHighlights())
                .inclusions(trek.getInclusions())
                .exclusions(trek.getExclusions())
                ...
                .itinerary(trek.getItinerary().stream()
                        .map(item -> TrekDetailResponse.ItineraryResponse.builder()
                                ...
                                .mealsIncluded(item.getMealsIncluded())
                ...
                .guides(
                        trek.getBatches().stream()
                                .flatMap(batch -> batch.getGuides().stream())
                                .distinct()
                                .sorted(Comparator.comparing(Guide::getName))
    ...
    private TrekAvailabilityResponse toAvailabilityResponse(Batch batch) {
        return TrekAvailabilityResponse.builder()
                ...
                .meetingPoint(
                        StringUtils.hasText(batch.getMeetingPoint())
                                ? batch.getMeetingPoint()
                                : batch.getTrek().getMeetingPoint()
                )
                .build();
    }
